...
import type { CreateCommand, PhoneBellRepository, PhoneRequestRecord } from "../domain/models";
...
export class PhoneRequestService {
  constructor(
    ...
    private readonly expiry: ExpiryScheduler,
  ) {}
  ...
  async create(command: CreateCommand): Promise<CreatePhoneRequestResponse> {
    ...
    try {
      await this.expiry.enqueue(reserved.request.requestId, reserved.request.expiresAt);
    } catch (error) {
      await this.repository.recordEvent(reserved.request.requestId, "EXPIRY_TASK_ENQUEUE_FAILED", {
    ...
    try {
      ...
      if (sent.status === "failed" || sent.status === "expired") {
        // A slow send can race the expiry task. Do not report a terminal
...
function safeErrorMessage(error: unknown): string {
  ...
  if (error instanceof Error) {
    ...
    // useful diagnostics while preventing accidental token/credential
    // disclosure if an SDK includes request details in its message.
